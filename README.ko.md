...
| 프로바이더 | 모델 |
|----------|------|
| **Groq (클라우드 STT)** | `whisper-large-v3-turbo` |
| **OpenAI (클라우드 LLM)** | `gpt-5.4` (기본), `gpt-5.4-mini`, `gpt-5.3-codex`, `gpt-5.3-codex-spark`, `gpt-5.2-codex` |
| **WhisperKit (로컬 STT)** | `openai_whisper-large-v3_turbo` (CoreML + ANE 최적화) |
| **MLX Audio (로컬 STT)** | `Qwen3-ASR-1.7B-8bit` (Python worker 기본값, 다른 mlx-audio 모델로 변경 가능) |
...
[GitHub Releases](https://github.com/Arsture/whispree/releases)에서 최신 `.zip`을 다운로드하세요. 공증되지 않은 앱이므로 첫 실행 시 우클릭 > 열기로 Gatekeeper를 우회하세요.

> **참고:** `.dmg`도 제공되지만 macOS Gatekeeper에 의해 차단될 수 있습니다. `.zip` 사용을 권장합니다.
