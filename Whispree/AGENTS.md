...
| `Services/` | 비즈니스 로직 서비스 8개 (see `Services/AGENTS.md`) |
...
```
Recording: Hotkey(Ctrl+Shift+R) → AudioService (record + FFT)
  → STTProvider.transcribe() → LLMProvider.correct() → TextInsertionService.insertText()

Quick Fix: Hotkey(Ctrl+Shift+D) → QuickFixService.captureSelectedText()
...
- 주요 SPM 의존성: WhisperKit 0.9.0, mlx-swift-lm (main), KeyboardShortcuts, LaunchAtLogin, Sparkle

### Concurrency Model
- `@MainActor`: AppState, RecordingCoordinator, AudioService, AppDelegate, LLMProvider
- **NOT @MainActor**: STTProvider (WhisperKit/Groq/MLX Audio) — ML 추론은 백그라운드
