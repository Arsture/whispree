...
- **외부 앱 컨텍스트 복원 아키텍처** (`Whispree/Models/ExternalContext.swift`): `.app` / `.chromeTab` / `.iTerm2Session` sum type. `RecordingCoordinator.startRecording()`에서 frontmost 앱별로 캡처 서비스 분기(`BrowserContextService.isChrome` / `TerminalContextService.isITerm2`) → `processPipeline()` Step 4에서 역분기 restore. 새 터미널/앱 지원 추가 시 enum case + 분기 2곳 + AGENTS.md 업데이트.
