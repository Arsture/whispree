...
릴리스에서 받은 `Whispree.app`을 `/Applications`에 두고 실행하면 메뉴바에 마이크 아이콘이 나타납니다. (Dock 아이콘과 메인 윈도우도 함께 표시됩니다.)

:::note
직접 빌드해서 쓰는 개발자라면 저장소의 `CLAUDE.md` 빌드 절차를 따르세요. 로컬 Xcode 서명 빌드는 Sparkle 자동 업데이트가 동작하지 않습니다(개발자는 `git pull` + 재빌드로 업데이트).
:::
